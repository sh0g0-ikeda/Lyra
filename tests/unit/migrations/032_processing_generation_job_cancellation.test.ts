...
describe('032 processing generation-job cancellation migration', () => {
  it('stores only server-derived processing cancellation metadata and indexes pending checkpoints', async () => {
    const sql = await readFile(
      join(process.cwd(), 'migrations', '032_add_processing_generation_job_cancellation.sql'),
      'utf8',
    );
    const normalized = sql.toLowerCase();

    expect(normalized).toContain('add column if not exists cancel_requested_at timestamptz');
    expect(normalized).toContain('add column if not exists cancel_requested_by_user_id uuid');
