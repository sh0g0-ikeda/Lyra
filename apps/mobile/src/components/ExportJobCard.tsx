...
interface ExportJobCardProps {
  ...
  language: 'ja' | 'en';
  onDownload?: (downloadUrl: string, job: ExportJobRecord) => void | Promise<void>;
}
  ...
  return (
    <View style={[styles.card, job.status === 'failed' ? styles.failedCard : null, isCompleted ? styles.completedCard : null]}>
      <View style={styles.header}>
        <Text style={styles.title}>{job.filename}</Text>
        <Text style={[styles.status, statusStyle(job.status)]}>{statusLabel(job.status, language)}</Text>
      </View>
      <Text style={styles.text}>{formatLabel(job.format, language)}</Text>
      {isCompleted || job.status === 'failed' || job.status === 'canceled' ? null : (
        <>
          <Text style={styles.text}>{stageLabel(job.progress_stage, language)}</Text>
          <View style={styles.progressTrack}>
            <View style={[styles.progressFill, { width: `${job.progress_percent}%` }]} />
          </View>
          <Text style={styles.progressPercent}>{job.progress_percent}%</Text>
        </>
      )}
...
function formatLabel(format: ExportJobRecord['format'], language: 'ja' | 'en'): string {
  return format === 'pdf'
...
function failureMessage(messageKey: string | null, language: 'ja' | 'en'): string {
  if (messageKey === 'export.error.failed') {
    return t(language, "generated.components.ExportJobCard.export.failed.try.again.shortly.86d7d749");
  }
  return t(language, "generated.components.ExportJobCard.export.failed.2c3901da");
}
...
const styles = StyleSheet.create({
  ...
  error: { ...textStyles.body, color: '#FF9E96' },
  failedCard: { borderColor: 'rgba(244, 67, 54, 0.54)' },
  ...
  statusDanger: { backgroundColor: colors.dangerSurface, color: '#F77E75' },
