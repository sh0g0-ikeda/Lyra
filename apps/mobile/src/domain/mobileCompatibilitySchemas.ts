...
const accountDeletionStoreSubscriptionSchema = z
  .object({
    store: z.enum(['apple', 'google']),
...
const accountDeletionBlockerSchema = z.discriminatedUnion('code', [
  z
    .object({
      ...
      organizations: z
        .array(z.object({ id, name: z.string().min(1).max(120) }).strict())
        .max(25)
    })
    .strict(),
  z
...
export const accountDeletionPreviewSchema = z
  .object({
    ...
    active_store_subscriptions: z.array(accountDeletionStoreSubscriptionSchema).max(20),
