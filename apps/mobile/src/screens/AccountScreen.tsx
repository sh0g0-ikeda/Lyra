...
export function AccountScreen(): React.JSX.Element {
  ...
  const mobileStoreProductCatalogQuery = useQuery({
    enabled: session !== null && organizationId === null && nativeMobileStore !== null,
  ...
  const mobileStoreProducts = useMemo(
    () =>
      mobileStoreProductCatalogQuery.data === undefined
        ? null
  ...
  const deletionMutation = useMutation({
    mutationFn: () =>
      api.requestAccountDeletion({
        confirmation: 'DELETE',
        acknowledge_active_subscription: acknowledgeSubscription,
        acknowledge_confirmed_assets: acknowledgeAssets
      }),
      ...
      >
        {deletionPreviewQuery.data === undefined ? (
          ...
          <AccountDeletionReview
            ...
            acknowledgeSubscription={acknowledgeSubscription}
...
function AccountDeletionReview({
  ...
  const requiresSubscriptionAcknowledgement = preview.active_personal_subscription_count > 0;
  const requiresAssetAcknowledgement = preview.confirmed_personal_asset_count > 0;
  const canSubmit =
    !hasOwnerBlocker &&
    confirmation === 'DELETE' &&
    (!requiresSubscriptionAcknowledgement || acknowledgeSubscription) &&
    ...
    <View style={styles.deletionReview}>
      ...
      {requiresSubscriptionAcknowledgement ? (
        <AcknowledgementRow
          ...
          label={subscriptionDeletionAcknowledgement({
            activeMobileStoreSubscriptionCount:
              preview.active_mobile_store_subscription_count,
            activeStripeSubscriptionCount: preview.active_stripe_subscription_count,
            language
          })}
          onChange={onAcknowledgeSubscription}
        />
      ) : null}
      {requiresAssetAcknowledgement ? (
        <AcknowledgementRow
          checked={acknowledgeAssets}
          label={t(language, 'screen.account.acknowledgeAssetDeletion', {
            assetCount: preview.confirmed_personal_asset_count
          })}
