...
describe('Mobile feature visibility', () => {
  ...
  it('キャラ画面とページ画面が非公開フラグを表示条件に使う', () => {
    ...
    expect(app).toContain('pushNotificationsEnabled ? <PushNotificationCoordinator /> : null');
    expect(appState).toMatch(
      /pushNotificationsEnabled\s*\?\s*unregisterPushNotifications\(api\)\s*:\s*unregisterNativePushNotifications\(\)/u
    );
  });
});
