...
describe('account deletion API contract', () => {
  ...
  it('個人アカウント削除のプレビューを組織 ID なしで取得する', async () => {
    const fetchMock = vi.fn().mockResolvedValue(
      new Response(
        JSON.stringify({
          ...
          confirmed_personal_asset_count: 2
        }),
    ...
    expect(fetchMock.mock.calls[0]?.[0]).toMatch(/\/api\/account\/deletion-preview$/);
  ...
  it('削除が blocked の場合も安全な業務結果として返し、組織 ID を送らない', async () => {
    ...
    await expect(
      client.requestAccountDeletion({
        confirmation: 'DELETE',
        acknowledge_active_subscription: false,
        acknowledge_confirmed_assets: false
      })
