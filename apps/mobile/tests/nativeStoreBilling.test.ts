  ...
  const sdk: NativeStoreBillingSdk = {
    ...
    fetchProducts: vi.fn().mockResolvedValue([
      { id: 'lyra.credits.200', title: '200 credits', displayPrice: '$2.99', type: 'in-app' },
      {
        ...
        title: 'Standard',
        displayPrice: '$4.99',
        subscriptionOffers: [{ offerToken: 'google-base-plan', sku: 'lyra.standard.monthly' }],
        type: 'subs'
      }
    ]),
    finishTransaction,
...
describe('native store billing adapter', () => {
    ...
    vi.mocked(ExpoIap.fetchProducts).mockResolvedValueOnce([
      {
        ...
        subscriptionOfferDetailsAndroid: [{
          offerToken: 'google-base-plan',
        }],
        title: 'Standard',
        type: 'subs'
      } as unknown as ExpoIap.Product
    ]);

    const products = await createExpoIapSdk().fetchProducts({
  ...
  it('復元ではtransaction IDが異なっても同じstore proofを一度だけ送信してfinishする', async () => {
    const harness = createSdkHarness();
    const backend = createBackend();
    harness.sdk.getAvailablePurchases = vi.fn().mockResolvedValue([
      {
        id: 'google-restore-one',
        productId: 'lyra.credits.200',
        purchaseState: 'purchased',
        purchaseToken: 'same-google-proof',
        store: 'google'
      },
      {
        id: 'google-restore-two',
        productId: 'lyra.credits.200',
        purchaseState: 'purchased',
        purchaseToken: 'same-google-proof',
        store: 'google'
    ...
    expect(backend.restorePurchases).toHaveBeenCalledWith({
      ...
      googlePurchaseTokens: ['same-google-proof']
    });
    expect(harness.finishTransaction).toHaveBeenCalledTimes(1);
  });

  it('Apple environmentが不明なpurchaseはverifyもfinishもしない', async () => {
    const harness = createSdkHarness();
    const backend = createBackend();
    const adapter = createNativeStoreBillingAdapter({ backend, products, sdk: harness.sdk });
    await adapter.connect();
    ...
    await harness.emitPurchase({
      environmentIOS: null,
      id: 'apple-unknown-environment',
      productId: 'lyra.standard.monthly',
      purchaseState: 'purchased',
      purchaseToken: 'signed-unknown-environment',
      store: 'apple'
    });

    expect(backend.verifyApplePurchase).not.toHaveBeenCalled();
    expect(harness.finishTransaction).not.toHaveBeenCalled();
