...
- [ ] PR-F: Story / Characters / PagesのMobile UI
  ...
  - [x] Mobileキャラ参照画像生成 / job監視 / candidate確定を既存Entity APIだけで接続
    - 証跡: feature code head `8d175c3`。dirtyキャラを既存保存フローで確定後、既存`entity_generate`へ現在の保存内容または明示したimport候補を渡し、exact job ID / type / Entityを照合する。1〜3候補を認証付きでpreviewし、選択・primary・明示確認後だけ既存confirm APIで確定する
