...
実装監査基準: `2a25fd7`（Mobileキャラ参照画像import / candidate preview / confirm feature code head、local full gate成功。GitHub CIは統合前にexact headで再確認）
...
- [ ] PR-F: Story / Characters / PagesのMobile UI
  ...
  - [x] Mobileキャラ参照画像のbase64 import / candidate preview / confirmを既存Entity APIだけで接続
    - 証跡: feature code head `2a25fd7`。保存済みEntityで端末画像を1枚選び、JPEG / PNG / WebPの実signatureと5 MiB上限を端末側でも検証し、1 creditを明示して既存import APIへ送る。候補tokenはcomponent memoryだけに保持し、認証付き・token非cache-keyでpreviewする。明示確認後だけ既存confirm APIへtokenと解析済みprompt supplementを送り、既存参照画像を残して新画像をprimaryにする
