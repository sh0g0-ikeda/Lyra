...
- [ ] PR-F: Story / Characters / PagesのMobile UI
  ...
  - 残り: 生成fileを削除可能にするdurable asset cleanup、Backend endpointがない作品削除・並べ替え、Scene削除Backend安全境界、Charactersの削除Backend安全境界・参照画像direct upload client / 本番設定・`costume_ref_id`のBackend整合性付き選択/変更、Panel作成・削除・並べ替え・entity assignment Mobile UI・frame / balloon、Page画像生成、organization workspace UI。PR-F全体は未完了のまま維持する
