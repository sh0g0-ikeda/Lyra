...
export interface StorePurchaseRepository {
  ...
  createPurchase(
    input: CreateStorePurchaseInput,
    client: DatabaseClient,
  ...
  hasActiveStripeConsumerSubscription(
    userId: string,
    client: DatabaseClient,
  ): Promise<boolean>;
  resolvePersonalPlan(
  ...
  findLatestStoreSubscriptionForUser(
    userId: string,
    client?: DatabaseClient,
...
export class PostgresStorePurchaseRepository implements StorePurchaseRepository {
  ...
  public async findUserForUpdate(
    userId: string,
    client: DatabaseClient,
  ): Promise<StorePurchaseUserRecord | null> {
  ...
  public async createPurchase(
    ...
    return mapStorePurchaseRow(requireRow(result.rows[0]));
  }
  ...
  public async addGrantedCredits(
    purchaseId: string,
    amount: number,
    client: DatabaseClient,
  ): Promise<void> {
  ...
  public async addReversedCredits(
    purchaseId: string,
    amount: number,
    client: DatabaseClient,
  ): Promise<void> {
  ...
  public async hasActiveStripeConsumerSubscription(
    userId: string,
    client: DatabaseClient,
  ): Promise<boolean> {
    const result = await client.query(
    ...
    const result = await client.query<{ plan_code: ConsumerPaidPlanCode }>(
      ...
      WITH stripe_candidate AS (
        ...
        WHERE user_id = $1
          ...
          AND plan_code IN ('standard', 'premium')
          AND status IN ('active', 'trialing')
        ORDER BY CASE plan_code WHEN 'premium' THEN 2 ELSE 1 END DESC
        LIMIT 1
      ),
      store_candidate AS (
        SELECT plan_code
      ...
      SELECT plan_code
      FROM stripe_candidate
      UNION ALL
      SELECT plan_code
      FROM store_candidate
      WHERE NOT EXISTS (SELECT 1 FROM stripe_candidate)
      LIMIT 1
      `,
      [userId],
    );
  ...
  public async findLatestStoreSubscriptionForUser(
    ...
    const queryClient = client ?? this.readClient;
    if (queryClient === undefined) {
      throw new Error('Store purchase read client is not configured');
    }
    const result = await queryClient.query<StoreSubscriptionSummaryRow>(
      ...
      ORDER BY
        CASE plan_code WHEN 'premium' THEN 2 ELSE 1 END DESC,
...
const STORE_PURCHASE_SELECT_SQL = `
  SELECT ${STORE_PURCHASE_FIELDS}
  FROM mobile_store_purchases
`;

function requireRow(row: StorePurchaseRow | undefined): StorePurchaseRow {
  if (row === undefined) {
