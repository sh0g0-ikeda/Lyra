...
import { entityReferenceUploadPresignBodySchema } from '../lib/validators/entity.schema.js';
...
export function createEntityReferenceUploadRoutes(
  ...
  app.post('/uploads/entity-reference/presign', async (c) => {
    ...
    const result = await dependencies.entityReferenceUploadService.createPresignedUpload(user.id, {
    ...
    return c.json(assertMobileResponseContract(entityReferenceUploadPresignResponseSchema, payload), 201);
  });
