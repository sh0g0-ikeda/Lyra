  ...
  .object({
    message: z
      .object({
        ...
        data: z.string().trim().min(1).max(131_072),
...
export function createMobilePurchaseWebhookRoutes(
  ...
  app.post('/apple', async (c) => {
    const body = parse(
      appleNotificationSchema.safeParse(
    ...
    await dependencies.mobileStorePurchaseService.handleAppleNotification(
      body.signedPayload,
    );
  ...
  app.post('/google', async (c) => {
    await dependencies.googlePubSubPushVerifier.verifyAuthorization(
      c.req.header('Authorization'),
    );
    const body = parse(
      googlePushSchema.safeParse(
        await readJsonBody(c, {
          maxBytes: REQUEST_BODY_LIMITS.DEFAULT_JSON_BYTES,
    ...
    await dependencies.mobileStorePurchaseService.handleGoogleRtdn({
      ...
      publishTime:
        body.message.publishTime === undefined
          ? null
