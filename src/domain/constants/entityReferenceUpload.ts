...
export function isEntityReferenceUploadMimeType(value: string): value is EntityReferenceUploadMimeType {
  return ENTITY_REFERENCE_UPLOAD_MIME_TYPES.includes(value as EntityReferenceUploadMimeType);
}

export function isEntityReferenceUploadSize(sizeBytes: number): boolean {
  return Number.isInteger(sizeBytes) && sizeBytes > 0 && sizeBytes <= ENTITY_IMPORT_MAX_FILE_SIZE_BYTES;
}
