...
export function createStoreProductCatalog(entries: readonly StoreProductDefinition[]): StoreProductCatalog {
  ...
  for (const entry of entries) {
    ...
    if (normalizedProductId.length === 0) {
      throw new ConfigurationError('Mobile store product mapping contains an empty product id');
    }
...
export function resolveStorePurchaseState(value: string): StorePurchaseState {
  if ((STORE_PURCHASE_STATES as readonly string[]).includes(value)) {
    return value as StorePurchaseState;
  }
...
export function createStoreIdentifierKey(secret: string, namespace: string, value: string): string {
  if (secret.trim().length < 32) {
    throw new ConfigurationError('Mobile store identifier key must be at least 32 characters');
  }

  return createHmac('sha256', secret).update(`${namespace}:${value}`, 'utf8').digest('base64url');
}
