...
export class CreditService implements CreditServicePort {
  ...
  public async grantSignupBonus(userId: string): Promise<CreditBalanceSnapshot> {
    return this.grantPurchasedCredits(userId, SIGNUP_BONUS_CREDITS, 'Initial free plan signup bonus');
  }
  ...
  private async grantPurchasedCredits(
    ...
    if (amount <= 0) {
      throw new ValidationError('Credit grant amount must be greater than zero');
    }
    ...
    return this.creditRepository.transaction(async (client) => {
      ...
      const nextBalance: CreditBalance = {
        ...currentBalance,
        purchasedCredits: currentBalance.purchasedCredits + amount,
      };
